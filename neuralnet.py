...
def random_init(shape):
    ...
    a = np.random.uniform(low=-0.1, high=0.1, size=shape)
    return a
...
class SoftMaxCrossEntropy:
    ...
    def forward(self, z: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        """
        softmaxarr = self._softmax(z)
        loss = self._cross_entropy(y,softmaxarr)
        return ((softmaxarr,loss))
...
class Linear:
    def __init__(self, input_size: int, output_size: int,
        ...
        self.weight = weight_init_fn((output_size,input_size))
    
        
        # TODO: set the bias terms to zero
        bias = np.zeros((output_size,1))
        self.weight = np.insert(self.weight,[0],bias,axis=1)
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        newx = np.concatenate((np.array([1]), x))
        self.cache.update({"x":newx})
        self.cache.update({"w":self.weight})
        ans = np.matmul(self.weight,newx)
        return ans
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        weightwbias = self.cache.get("w")
        weightwobias = weightwbias[:,1:]
        weightwobiasT = np.transpose(weightwobias)
        ans = np.matmul(weightwobiasT,dz)
        self.gradient = np.outer(dz,self.cache.get("x"))
...
class NN:
    def __init__(self, input_size: int, hidden_size: int, output_size: int,
        ...
        self.linearalpha = Linear(input_size,hidden_size,weight_init_fn,learning_rate)
        self.linearbeta = Linear(hidden_size,output_size,weight_init_fn,learning_rate)
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        a = self.linearalpha.forward(x)
        z = self.sig.forward(a)
        b = self.linearbeta.forward(z)
        return(self.softmax.forward(b,y))
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        dldz = self.linearbeta.backward(dldb)
        dlda = self.sig.backward(dldz)
        dldx = self.linearalpha.backward(dlda)
    ...
    def step(self):
        ...
        self.linearbeta.step()
        self.linearalpha.step()
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        for e in range(n_epochs):
            ...
            for i in range(len(D[0])):
                ...
                self.step()
            trainloss.append(self.compute_loss(X_tr,y_tr))
            testloss.append(self.compute_loss(X_test,y_test))
        # sum1 = 0
        # sum2 = 0
        ...
        plt.plot(epochs, trainloss, color ="green")
        plt.plot(epochs,testloss,color = "red")
        plt.show()
    ...
    def test(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        for i in range(len(y)):
            xi = X[i]
            yi = y[i]
            y_hat = self.forward(xi,yi)[0]
            ans[i] = np.argmax(y_hat)
            if(ans[i]!=y[i]):
                numErrors+=1
        return (ans,numErrors/len(y))
